...
export const showOddsLabels = (outcomeRegistryId: any, participants: any) => {
  const oddsLabels: any = {
    ...
    5: `${shortenCompetitorName(
      participants[0].name
    )} win or ${shortenCompetitorName(participants[1].name)} win`,
...
const fns = {
  W1: ({ participants }: Props) =>
    `${shortenCompetitorName(participants[0].name)} win`,
  W2: ({ participants }: Props) =>
    `${shortenCompetitorName(participants[1].name)} win`,
  Draw: (props: Props) => "Draw",
  "1X": ({ participants }: Props) =>
    `${shortenCompetitorName(participants[0].name)} win or draw`,
  "12": ({ participants }: Props) =>
    `${shortenCompetitorName(
      participants[0].name
    )} win or ${shortenCompetitorName(participants[1].name)} win`,
  "2X": ({ participants }: Props) =>
    `${shortenCompetitorName(participants[1].name)} win or draw`,
};
